...
const UserCodeScreen: React.FC<UserCodeScreenProps> = ({ onCodeSuccess, onGoBack }) => {
  ...
  // 영어+숫자만 허용하는 정규식
  const ONLY_ENG_NUM = /[^A-Za-z0-9]/g;

  /**
   * 📝 입력값 변경 핸들러
   * 영어와 숫자만 허용하고 자동으로 대문자로 변환합니다.
   */
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const next = e.target.value.replace(ONLY_ENG_NUM, '').toUpperCase();
    setCode(next);
    setError('');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 🔍 서버 API를 통한 코드 검증
      await verifyCodeApi({ code: code.trim().toUpperCase() });
      
      // ✅ 유효한 코드 → 트랜지션 화면 표시
      setShowTransition(true);
    } catch (error) {
      // ❌ 에러 처리 - 에러 유형에 따른 적절한 메시지 표시
        ...
        <div className={styles.cardPanel}>
            ...
            <div className={styles.formGroup}>
              ...
              <input
                ...
                onBeforeInput={(e) => {
                  ...
                  if (ch && /[^A-Za-z0-9]/.test(ch)) e.preventDefault(); // 영어/숫자 외 입력 차단
                }}
                pattern="[A-Za-z0-9]*"       // 제출 시에도 영어/숫자만 허용
          ...
          <div className={styles.helpSection}>
            ...
            <ul className={styles.helpList}>
              ...
              <li>문제가 있다면 관리자에게 문의하세요.(ranige0610@gmail.com)</li>
              <li>입력 시 영어 대문자와 숫자만 허용됩니다.</li>
